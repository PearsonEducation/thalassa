...
Client.prototype.socketConnect = function() {
  ...
  if (this.socket === null) {
    ...
    this.socket.on('message', function (regId, state, reg) {
      ...
      if (state === 'online') {
        //if we already have the regstration, and nothing changed, do nothing
        if(regId in self.registrations && self.registrations[regId] === reg){
          return;
        }
        self.registrations[regId] = reg;
