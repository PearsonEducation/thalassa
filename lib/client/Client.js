    ...
    this.socket.on('message', function (regId, state, reg) {
      ...
      if (state === 'online') {
        ...
        if(regId in self.registrations){
          //if we already have the registration, and nothing changed, do nothing
          if(JSON.stringify(updatedReg.meta) === JSON.stringify(self.registrations[regId]))
            return;
        }
        self.registrations[regId] = JSON.parse(reg);
