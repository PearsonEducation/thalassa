...
var Client = module.exports = function (opts) {
  ...
  this.ATTEMPTS_BEFORE_RETRY    = opts.attemptsBeforeRetry || 5;
...
Client.prototype.stop = function() {
  ...
  Object.keys(self.registrations).forEach(function (key) {
    var reg = self.registrations[key];
    ...
    this.socket.on('message', function (regId, state, reg) {
      ...
      if (state === 'online') {
        ...
        if(regId in self.registrations){
          ...
          if(deepEqual(updatedReg.meta, self.registrations[regId].meta))
            return;
        }
...
Client.prototype._sendHTTPUpdate = function (intent) {
  ...
  if(!(intent.id in self.attempts) || self.attempts[intent.id] == self.ATTEMPTS_BEFORE_RETRY){
    //If this is a case of hitting max attempts, log it
    if(intent.id in self.attempts){
      self.log('error', 'Reached max number of attempts (' + self.ATTEMPTS_BEFORE_RETRY + ') waiting for response, sending new request');
    }
    self.attempts[intent.id] = 0;
  }
  
  // If we have a request still pending, increment the number of attempts and return 
  if (self.attempts[intent.id] > 0) {
    self.attempts[intent.id]++;
    self.log('error', 'Thalassa:Client._sendHTTPUpdate call still pending! (skipping): ' + intent.id);
    return;
  }
